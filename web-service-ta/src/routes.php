...
return function (App $app) {
    ...
    $app->post('/sessionSignIn', function ($request, $response) {
        ...
        if($result==true){
            ...
            $sql = "SELECT u.id_user,u.password_user,u.nama_user,u.telpon_user,u.status_user,u.premium_available_until,u.lokasi_aktif_user,u.id_kecamatan_user,u.status_aktif_user,k.nama_kecamatan AS kecamatan_user
    ...
    $app->post('/getLaporanLostFoundWithFilter', function ($request, $response) {
        $body=$request->getParsedBody();
        $jenis_laporan=implode(",",$body["list_jenis_laporan"]);
        $tanggal_akhir=$body["tanggal_awal"];
        $tanggal_akhir=$body["tanggal_akhir"];
        $array_barang=implode(",",$body["list_id_barang"]);
        $array_kecamatan=implode(",",$body["list_id_kecamatan"]);
